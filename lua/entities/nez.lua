...
local nez_spawn_protect = CreateConVar("nez_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, nez will not target players or hide within 200 units of a spawn point.")

local nez_attack_distance = CreateConVar("nez_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of nez's attack.")

local nez_attack_interval = CreateConVar("nez_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between nez's attacks.")

local nez_attack_force = CreateConVar("nez_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of nez's attack. Higher values throw things farther.")

local nez_smash_props = CreateConVar("nez_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, nez will punch through any props placed in their way.")

local nez_hiding_scan_interval = CreateConVar("nez_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "nez will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant nezs are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local nez_hiding_repath_interval = CreateConVar("nez_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to nez's hiding spot will be redetermined every X seconds.")

local nez_chase_repath_interval = CreateConVar("nez_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of nez's target will be redetermined every X seconds.")

local nez_expensive_scan_interval = CreateConVar("nez_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local nez_force_download = CreateConVar("nez_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download nez resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear nez!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (nez_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("nez_nag")
util.AddNetworkString("nez_nagresponse")
util.AddNetworkString("nez_navgen")
...
local VECTOR_nez_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_nez_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("nez: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other nezs, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "nezMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "nez") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.nez_HasBeenNagged) then return end
	ply.nez_HasBeenNagged = true

	-- Try to explain why nez isn't working.
	net.Start("nez_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("nez: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("nez: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("nez_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.nez_HasBeenNagged = false
		return
	end
	...
	--for _, nez in pairs(ents.FindByClass("nez")) do
	--	nez:Remove()
	--end

	print("nez: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "nezNavGen", navEndGenerate)
	else
		print("nez: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = nez_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `nez_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (nez_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (nez_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. nez means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * nez_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * nez_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not nez_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (nez_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > nez_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > nez_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(nez_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > nez_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= nez_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= nez_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= nez_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= nez_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local nezsLastFrame = 0
hook.Add("Tick", "nezBenchmark", function()
	local now = SysTime()
	local numnezs = #ents.FindByClass("nez")

	if (timeAll ~= 0) then

		local nezTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    nezTime / nezsLastFrame, nezTime, frameTime, (nezTime / frameTime) * 100, nezsLastFrame, numThink, (numThink / nezsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	nezsLastFrame = numnezs
end)]]
...
killicon.Add("nez", "nez/nez_killicon", color_white)
language.Add("nez", "neznakomov")
...
local nezMusic = nil
local lastPanic = 0 -- The last time we were in music range of a nez.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local nez_music_volume = CreateConVar("nez_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by nez. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another nez comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, nezs do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_nez_PANIC_COUNT nezs are this close, or an equivalent score.
local MUSIC_nez_PANIC_COUNT = 8 -- That's a lot of nez.

local MUSIC_nez_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_nez_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("nez") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("neznezMusicUpdate")

		if (nezMusic ~= nil) then
			nezMusic:Stop()
		end
	...
	if (nezMusic == nil) then
		if (IsValid(LocalPlayer())) then
			nezMusic = CreateSound(LocalPlayer(), "nez/panic.mp3")
			nezMusic:Stop()
		else
	...
	if (nez_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		nezMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "nez") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_nez_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear nez taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(nez_music_volume:GetFloat(), 0, 1))

	nezMusic:Play()
	nezMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	nezMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("neznezMusicUpdate")) then
		timer.Create("neznezMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local nezMaterial = Material("nez/nez.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(nezMaterial)
...
hook.Add("OnEntityCreated", "nezInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "nez") then return end

	local nezEntTable = scripted_ents.GetStored("nez")

	table.Merge(ent, nezEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("nez_removed", nezDeregister)
end)

hook.Add("NetworkEntityCreated", "nezNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "nez") then return end
...
surface.CreateFont("nezHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("nezHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("nez is studying this map.", "nezHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "nezHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "nezHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "nezHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "nezHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("nez_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "nezNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. nez doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for nez to figure this map out.
...
-- Lazy
net.Receive("nez_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! nez doesn't know this map.
Would you like nez to learn it?]],
		            "This map is currently not nez-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! nez doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to nez.]],
		            "This map is currently not nez-compatible!",
		            "Ok", nagAgain,
