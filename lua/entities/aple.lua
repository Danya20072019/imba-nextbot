...
util.PrecacheSound("aple/jump.mp3")
util.PrecacheSound("aple/panic.mp3")
util.PrecacheSound("aple/pieceofcake.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local aple_acquire_distance = CreateConVar("aple_acquire_distance", 999999999, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which aple will chase a target.")

local aple_spawn_protect = CreateConVar("aple_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, aple will not target players or hide within 200 units of a spawn point.")

local aple_attack_distance = CreateConVar("aple_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of aple's attack.")

local aple_attack_interval = CreateConVar("aple_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between aple's attacks.")

local aple_attack_force = CreateConVar("aple_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of aple's attack. Higher values throw things farther.")

local aple_smash_props = CreateConVar("aple_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, aple will punch through any props placed in their way.")

local aple_hiding_scan_interval = CreateConVar("aple_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "aple will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant aples are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local aple_hiding_repath_interval = CreateConVar("aple_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to aple's hiding spot will be redetermined every X seconds.")

local aple_chase_repath_interval = CreateConVar("aple_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of aple's target will be redetermined every X seconds.")

local aple_expensive_scan_interval = CreateConVar("aple_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local aple_force_download = CreateConVar("aple_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download aple resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear aple!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (aple_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("aple_nag")
util.AddNetworkString("aple_nagresponse")
util.AddNetworkString("aple_navgen")
...
local VECTOR_aple_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_aple_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("aple: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other aples, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "apleMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "aple") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.aple_HasBeenNagged) then return end
	ply.aple_HasBeenNagged = true

	-- Try to explain why aple isn't working.
	net.Start("aple_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("aple: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("aple: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("aple_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.aple_HasBeenNagged = false
		return
	end
	...
	--for _, aple in pairs(ents.FindByClass("aple")) do
	--	aple:Remove()
	--end

	print("aple: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "apleNavGen", navEndGenerate)
	else
		print("aple: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = aple_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `aple_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (aple_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (aple_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. aple means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * aple_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * aple_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not aple_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (aple_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = aple_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "aple/jump.mp3" or "aple/jump.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > aple_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > aple_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(aple_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > aple_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= aple_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= aple_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= aple_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= aple_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local aplesLastFrame = 0
hook.Add("Tick", "apleBenchmark", function()
	local now = SysTime()
	local numaples = #ents.FindByClass("aple")

	if (timeAll ~= 0) then

		local apleTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    apleTime / aplesLastFrame, apleTime, frameTime, (apleTime / frameTime) * 100, aplesLastFrame, numThink, (numThink / aplesLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	aplesLastFrame = numaples
end)]]
...
killicon.Add("aple", "aple/aple_killicon", color_white)
language.Add("aple", "aple")
...
local apleMusic = nil
local lastPanic = 0 -- The last time we were in music range of a aple.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local aple_music_volume = CreateConVar("aple_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by aple. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another aple comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, aples do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_aple_PANIC_COUNT aples are this close, or an equivalent score.
local MUSIC_aple_PANIC_COUNT = 8 -- That's a lot of aple.

local MUSIC_aple_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_aple_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("aple") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("apleapleMusicUpdate")

		if (apleMusic ~= nil) then
			apleMusic:Stop()
		end
	...
	if (apleMusic == nil) then
		if (IsValid(LocalPlayer())) then
			apleMusic = CreateSound(LocalPlayer(), "aple/panic.mp3")
			apleMusic:Stop()
		else
	...
	if (aple_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		apleMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "aple") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_aple_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear aple taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(aple_music_volume:GetFloat(), 0, 1))

	apleMusic:Play()
	apleMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	apleMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("apleapleMusicUpdate")) then
		timer.Create("apleapleMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local apleMaterial = Material("aple/aple.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(apleMaterial)
...
hook.Add("OnEntityCreated", "apleInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "aple") then return end

	local apleEntTable = scripted_ents.GetStored("aple")

	table.Merge(ent, apleEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("aple_removed", apleDeregister)
end)

hook.Add("NetworkEntityCreated", "apleNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "aple") then return end
...
surface.CreateFont("apleHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("apleHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("aple is studying this map.", "apleHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "apleHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "apleHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "apleHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "apleHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("aple_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "apleNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. aple doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for aple to figure this map out.
...
-- Lazy
net.Receive("aple_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! aple doesn't know this map.
Would you like aple to learn it?]],
		            "This map is currently not aple-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! aple doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to aple.]],
		            "This map is currently not aple-compatible!",
		            "Ok", nagAgain,
