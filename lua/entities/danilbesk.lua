...
util.PrecacheSound("danilbesk/panic.mp3")
util.PrecacheSound("danilbesk/pieceofcake.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local danilbesk_acquire_distance = CreateConVar("danilbesk_acquire_distance", 999999999, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which danilbesk will chase a target.")

local danilbesk_spawn_protect = CreateConVar("danilbesk_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, danilbesk will not target players or hide within 200 units of a spawn point.")

local danilbesk_attack_distance = CreateConVar("danilbesk_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of danilbesk's attack.")

local danilbesk_attack_interval = CreateConVar("danilbesk_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between danilbesk's attacks.")

local danilbesk_attack_force = CreateConVar("danilbesk_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of danilbesk's attack. Higher values throw things farther.")

local danilbesk_smash_props = CreateConVar("danilbesk_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, danilbesk will punch through any props placed in their way.")

local danilbesk_hiding_scan_interval = CreateConVar("danilbesk_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "danilbesk will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant danilbesks are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local danilbesk_hiding_repath_interval = CreateConVar("danilbesk_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to danilbesk's hiding spot will be redetermined every X seconds.")

local danilbesk_chase_repath_interval = CreateConVar("danilbesk_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of danilbesk's target will be redetermined every X seconds.")

local danilbesk_expensive_scan_interval = CreateConVar("danilbesk_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local danilbesk_force_download = CreateConVar("danilbesk_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download danilbesk resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear danilbesk!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (danilbesk_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("danilbesk_nag")
util.AddNetworkString("danilbesk_nagresponse")
util.AddNetworkString("danilbesk_navgen")
...
local VECTOR_danilbesk_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_danilbesk_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("danilbesk: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other danilbesks, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "danilbeskMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "danilbesk") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.danilbesk_HasBeenNagged) then return end
	ply.danilbesk_HasBeenNagged = true

	-- Try to explain why danilbesk isn't working.
	net.Start("danilbesk_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("danilbesk: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("danilbesk: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("danilbesk_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.danilbesk_HasBeenNagged = false
		return
	end
	...
	--for _, danilbesk in pairs(ents.FindByClass("danilbesk")) do
	--	danilbesk:Remove()
	--end

	print("danilbesk: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "danilbeskNavGen", navEndGenerate)
	else
		print("danilbesk: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = danilbesk_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `danilbesk_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (danilbesk_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (danilbesk_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. danilbesk means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * danilbesk_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * danilbesk_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not danilbesk_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (danilbesk_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = danilbesk_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > danilbesk_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > danilbesk_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(danilbesk_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > danilbesk_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= danilbesk_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= danilbesk_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= danilbesk_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= danilbesk_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local danilbesksLastFrame = 0
hook.Add("Tick", "danilbeskBenchmark", function()
	local now = SysTime()
	local numdanilbesks = #ents.FindByClass("danilbesk")

	if (timeAll ~= 0) then

		local danilbeskTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    danilbeskTime / danilbesksLastFrame, danilbeskTime, frameTime, (danilbeskTime / frameTime) * 100, danilbesksLastFrame, numThink, (numThink / danilbesksLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	danilbesksLastFrame = numdanilbesks
end)]]
...
killicon.Add("danilbesk", "danilbesk/danilbesk_killicon", color_white)
language.Add("danilbesk", "Danil Besk")
...
local danilbeskMusic = nil
local lastPanic = 0 -- The last time we were in music range of a danilbesk.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local danilbesk_music_volume = CreateConVar("danilbesk_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by danilbesk. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another danilbesk comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, danilbesks do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_danilbesk_PANIC_COUNT danilbesks are this close, or an equivalent score.
local MUSIC_danilbesk_PANIC_COUNT = 8 -- That's a lot of danilbesk.

local MUSIC_danilbesk_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_danilbesk_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("danilbesk") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("danilbeskdanilbeskMusicUpdate")

		if (danilbeskMusic ~= nil) then
			danilbeskMusic:Stop()
		end
	...
	if (danilbeskMusic == nil) then
		if (IsValid(LocalPlayer())) then
			danilbeskMusic = CreateSound(LocalPlayer(), "danilbesk/panic.mp3")
			danilbeskMusic:Stop()
		else
	...
	if (danilbesk_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		danilbeskMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "danilbesk") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_danilbesk_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear danilbesk taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(danilbesk_music_volume:GetFloat(), 0, 1))

	danilbeskMusic:Play()
	danilbeskMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	danilbeskMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("danilbeskdanilbeskMusicUpdate")) then
		timer.Create("danilbeskdanilbeskMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local danilbeskMaterial = Material("danilbesk/danilbesk.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(danilbeskMaterial)
...
hook.Add("OnEntityCreated", "danilbeskInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "danilbesk") then return end

	local danilbeskEntTable = scripted_ents.GetStored("danilbesk")

	table.Merge(ent, danilbeskEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("danilbesk_removed", danilbeskDeregister)
end)

hook.Add("NetworkEntityCreated", "danilbeskNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "danilbesk") then return end
...
surface.CreateFont("danilbeskHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("danilbeskHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("danilbesk is studying this map.", "danilbeskHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "danilbeskHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "danilbeskHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "danilbeskHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "danilbeskHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("danilbesk_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "danilbeskNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. danilbesk doesn't even know where to start with this map.
...
local function navGenerate()
	net.Start("danilbesk_nagresponse")
		net.WriteBit(true)
	net.SendToServer()
end
...
local function navWarning()
	Derma_Query([[It will take a while for danilbesk to figure this map out.
...
-- Lazy
net.Receive("danilbesk_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! danilbesk doesn't know this map.
Would you like danilbesk to learn it?]],
		            "This map is currently not danilbesk-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! danilbesk doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to danilbesk.]],
		            "This map is currently not danilbesk-compatible!",
		            "Ok", nagAgain,
