...
util.PrecacheSound("xiavier/jump.mp3")
util.PrecacheSound("xiavier/panic.mp3")
util.PrecacheSound("xiavier/fnaf-1-jumpscare.mp3")
util.PrecacheSound("xiavier/fnaf-4-jumpscare.mp3")
util.PrecacheSound("xiavier/fnaf-6-jumpscare.mp3")
...
local xiavier_spawn_protect = CreateConVar("xiavier_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, xiavier will not target players or hide within 200 units of a spawn point.")

local xiavier_attack_distance = CreateConVar("xiavier_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of xiavier's attack.")

local xiavier_attack_interval = CreateConVar("xiavier_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between xiavier's attacks.")

local xiavier_attack_force = CreateConVar("xiavier_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of xiavier's attack. Higher values throw things farther.")

local xiavier_smash_props = CreateConVar("xiavier_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, xiavier will punch through any props placed in their way.")

local xiavier_hiding_scan_interval = CreateConVar("xiavier_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "xiavier will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant xiaviers are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local xiavier_hiding_repath_interval = CreateConVar("xiavier_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to xiavier's hiding spot will be redetermined every X seconds.")

local xiavier_chase_repath_interval = CreateConVar("xiavier_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of xiavier's target will be redetermined every X seconds.")

local xiavier_expensive_scan_interval = CreateConVar("xiavier_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local xiavier_force_download = CreateConVar("xiavier_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download xiavier resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear xiavier!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (xiavier_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("xiavier_nag")
util.AddNetworkString("xiavier_nagresponse")
util.AddNetworkString("xiavier_navgen")
...
local VECTOR_xiavier_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_xiavier_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("xiavier: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other xiaviers, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "xiavierMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "xiavier") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.xiavier_HasBeenNagged) then return end
	ply.xiavier_HasBeenNagged = true

	-- Try to explain why xiavier isn't working.
	net.Start("xiavier_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("xiavier: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("xiavier: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("xiavier_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.xiavier_HasBeenNagged = false
		return
	end
	...
	--for _, xiavier in pairs(ents.FindByClass("xiavier")) do
	--	xiavier:Remove()
	--end

	print("xiavier: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "xiavierNavGen", navEndGenerate)
	else
		print("xiavier: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = xiavier_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `xiavier_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (xiavier_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (xiavier_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. xiavier means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * xiavier_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * xiavier_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not xiavier_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (xiavier_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = xiavier_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "xiavier/jump.mp3" or "xiavier/jump.mp3"), 350, 100)
	end
end
...
local tauntSounds = {
	"xiavier/fnaf-1-jumpscare.mp3",
	"xiavier/fnaf-4-jumpscare.mp3",
	"xiavier/fnaf-6-jumpscare.mp3"
}
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > xiavier_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > xiavier_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(xiavier_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > xiavier_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= xiavier_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= xiavier_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= xiavier_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= xiavier_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local xiaviersLastFrame = 0
hook.Add("Tick", "xiavierBenchmark", function()
	local now = SysTime()
	local numxiaviers = #ents.FindByClass("xiavier")

	if (timeAll ~= 0) then

		local xiavierTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    xiavierTime / xiaviersLastFrame, xiavierTime, frameTime, (xiavierTime / frameTime) * 100, xiaviersLastFrame, numThink, (numThink / xiaviersLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	xiaviersLastFrame = numxiaviers
end)]]
...
killicon.Add("xiavier", "xiavier/xiavier_killicon", color_white)
language.Add("xiavier", "xiavier")
...
local xiavierMusic = nil
local lastPanic = 0 -- The last time we were in music range of a xiavier.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local xiavier_music_volume = CreateConVar("xiavier_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by xiavier. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another xiavier comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, xiaviers do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_xiavier_PANIC_COUNT xiaviers are this close, or an equivalent score.
local MUSIC_xiavier_PANIC_COUNT = 8 -- That's a lot of xiavier.

local MUSIC_xiavier_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_xiavier_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("xiavier") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("xiavierxiavierMusicUpdate")

		if (xiavierMusic ~= nil) then
			xiavierMusic:Stop()
		end
	...
	if (xiavierMusic == nil) then
		if (IsValid(LocalPlayer())) then
			xiavierMusic = CreateSound(LocalPlayer(), "xiavier/panic.mp3")
			xiavierMusic:Stop()
		else
	...
	if (xiavier_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		xiavierMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "xiavier") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_xiavier_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear xiavier taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(xiavier_music_volume:GetFloat(), 0, 1))

	xiavierMusic:Play()
	xiavierMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	xiavierMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("xiavierxiavierMusicUpdate")) then
		timer.Create("xiavierxiavierMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local xiavierMaterial = Material("xiavier/xiavier.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(xiavierMaterial)
...
hook.Add("OnEntityCreated", "xiavierInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "xiavier") then return end

	local xiavierEntTable = scripted_ents.GetStored("xiavier")

	table.Merge(ent, xiavierEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("xiavier_removed", xiavierDeregister)
end)

hook.Add("NetworkEntityCreated", "xiavierNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "xiavier") then return end
...
surface.CreateFont("xiavierHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("xiavierHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("xiavier is studying this map.", "xiavierHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "xiavierHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "xiavierHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "xiavierHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "xiavierHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("xiavier_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "xiavierNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. xiavier doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for xiavier to figure this map out.
...
-- Lazy
net.Receive("xiavier_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! xiavier doesn't know this map.
Would you like xiavier to learn it?]],
		            "This map is currently not xiavier-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! xiavier doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to xiavier.]],
		            "This map is currently not xiavier-compatible!",
		            "Ok", nagAgain,
