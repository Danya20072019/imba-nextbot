...
util.PrecacheSound("bbq/jump.mp3")
util.PrecacheSound("bbq/panic.mp3")
util.PrecacheSound("bbq/pieceofcake.mp3")
...
local bbq_spawn_protect = CreateConVar("bbq_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, bbq will not target players or hide within 200 units of a spawn point.")

local bbq_attack_distance = CreateConVar("bbq_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of bbq's attack.")

local bbq_attack_interval = CreateConVar("bbq_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between bbq's attacks.")

local bbq_attack_force = CreateConVar("bbq_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of bbq's attack. Higher values throw things farther.")

local bbq_smash_props = CreateConVar("bbq_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, bbq will punch through any props placed in their way.")

local bbq_hiding_scan_interval = CreateConVar("bbq_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "bbq will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant bbqs are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local bbq_hiding_repath_interval = CreateConVar("bbq_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to bbq's hiding spot will be redetermined every X seconds.")

local bbq_chase_repath_interval = CreateConVar("bbq_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of bbq's target will be redetermined every X seconds.")

local bbq_expensive_scan_interval = CreateConVar("bbq_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local bbq_force_download = CreateConVar("bbq_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download bbq resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear bbq!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (bbq_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("bbq_nag")
util.AddNetworkString("bbq_nagresponse")
util.AddNetworkString("bbq_navgen")
...
local VECTOR_bbq_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_bbq_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("bbq: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other bbqs, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "bbqMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "bbq") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.bbq_HasBeenNagged) then return end
	ply.bbq_HasBeenNagged = true

	-- Try to explain why bbq isn't working.
	net.Start("bbq_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("bbq: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("bbq: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("bbq_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.bbq_HasBeenNagged = false
		return
	end
	...
	--for _, bbq in pairs(ents.FindByClass("bbq")) do
	--	bbq:Remove()
	--end

	print("bbq: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "bbqNavGen", navEndGenerate)
	else
		print("bbq: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = bbq_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `bbq_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (bbq_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (bbq_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. bbq means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * bbq_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * bbq_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not bbq_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (bbq_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = bbq_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "bbq/jump.mp3" or "bbq/jump.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > bbq_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > bbq_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(bbq_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > bbq_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= bbq_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= bbq_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= bbq_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= bbq_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local bbqsLastFrame = 0
hook.Add("Tick", "bbqBenchmark", function()
	local now = SysTime()
	local numbbqs = #ents.FindByClass("bbq")

	if (timeAll ~= 0) then

		local bbqTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    bbqTime / bbqsLastFrame, bbqTime, frameTime, (bbqTime / frameTime) * 100, bbqsLastFrame, numThink, (numThink / bbqsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	bbqsLastFrame = numbbqs
end)]]
...
killicon.Add("bbq", "bbq/bbq_killicon", color_white)
language.Add("bbq", "bbq")
...
local bbqMusic = nil
local lastPanic = 0 -- The last time we were in music range of a bbq.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local bbq_music_volume = CreateConVar("bbq_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by bbq. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another bbq comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, bbqs do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_bbq_PANIC_COUNT bbqs are this close, or an equivalent score.
local MUSIC_bbq_PANIC_COUNT = 8 -- That's a lot of bbq.

local MUSIC_bbq_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_bbq_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("bbq") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("bbqbbqMusicUpdate")

		if (bbqMusic ~= nil) then
			bbqMusic:Stop()
		end
	...
	if (bbqMusic == nil) then
		if (IsValid(LocalPlayer())) then
			bbqMusic = CreateSound(LocalPlayer(), "bbq/panic.mp3")
			bbqMusic:Stop()
		else
	...
	if (bbq_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		bbqMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "bbq") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_bbq_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear bbq taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(bbq_music_volume:GetFloat(), 0, 1))

	bbqMusic:Play()
	bbqMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	bbqMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("bbqbbqMusicUpdate")) then
		timer.Create("bbqbbqMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local bbqMaterial = Material("bbq/bbq.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(bbqMaterial)
...
hook.Add("OnEntityCreated", "bbqInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "bbq") then return end

	local bbqEntTable = scripted_ents.GetStored("bbq")

	table.Merge(ent, bbqEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("bbq_removed", bbqDeregister)
end)

hook.Add("NetworkEntityCreated", "bbqNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "bbq") then return end
...
surface.CreateFont("bbqHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("bbqHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("bbq is studying this map.", "bbqHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "bbqHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "bbqHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "bbqHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "bbqHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("bbq_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "bbqNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. bbq doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for bbq to figure this map out.
...
-- Lazy
net.Receive("bbq_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! bbq doesn't know this map.
Would you like bbq to learn it?]],
		            "This map is currently not bbq-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! bbq doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to bbq.]],
		            "This map is currently not bbq-compatible!",
		            "Ok", nagAgain,
