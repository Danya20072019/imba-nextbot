...
util.PrecacheSound("ray/jump.mp3")
util.PrecacheSound("ray/panic.mp3")
util.PrecacheSound("ray/spring.mp3")
util.PrecacheSound("ray/stepitup.mp3")
util.PrecacheSound("ray/tooslow.ogg")
util.PrecacheSound("ray/tooeasy.mp3")
util.PrecacheSound("ray/pieceofcake.mp3")
...
local ray_spawn_protect = CreateConVar("ray_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, ray will not target players or hide within 200 units of a spawn point.")

local ray_attack_distance = CreateConVar("ray_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of ray's attack.")

local ray_attack_interval = CreateConVar("ray_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between ray's attacks.")

local ray_attack_force = CreateConVar("ray_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of ray's attack. Higher values throw things farther.")

local ray_smash_props = CreateConVar("ray_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, ray will punch through any props placed in their way.")

local ray_hiding_scan_interval = CreateConVar("ray_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "ray will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant rays are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local ray_hiding_repath_interval = CreateConVar("ray_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to ray's hiding spot will be redetermined every X seconds.")

local ray_chase_repath_interval = CreateConVar("ray_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of ray's target will be redetermined every X seconds.")

local ray_expensive_scan_interval = CreateConVar("ray_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local ray_force_download = CreateConVar("ray_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download ray resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear ray!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (ray_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("ray_nag")
util.AddNetworkString("ray_nagresponse")
util.AddNetworkString("ray_navgen")
...
local VECTOR_ray_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_ray_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("ray: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other rays, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "rayMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "ray") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.ray_HasBeenNagged) then return end
	ply.ray_HasBeenNagged = true

	-- Try to explain why ray isn't working.
	net.Start("ray_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("ray: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("ray: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("ray_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.ray_HasBeenNagged = false
		return
	end
	...
	--for _, ray in pairs(ents.FindByClass("ray")) do
	--	ray:Remove()
	--end

	print("ray: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "rayNavGen", navEndGenerate)
	else
		print("ray: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = ray_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `ray_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (ray_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (ray_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. ray means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * ray_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * ray_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not ray_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (ray_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = ray_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "ray/spring.mp3" or "ray/jump.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > ray_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > ray_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(ray_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > ray_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= ray_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= ray_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= ray_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= ray_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local raysLastFrame = 0
hook.Add("Tick", "rayBenchmark", function()
	local now = SysTime()
	local numrays = #ents.FindByClass("ray")

	if (timeAll ~= 0) then

		local rayTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    rayTime / raysLastFrame, rayTime, frameTime, (rayTime / frameTime) * 100, raysLastFrame, numThink, (numThink / raysLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	raysLastFrame = numrays
end)]]
...
killicon.Add("ray", "ray/ray_killicon", color_white)
language.Add("ray", "RED PEDOPHILE")
...
local RayMusic = nil
local lastPanic = 0 -- The last time we were in music range of a ray.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local ray_music_volume = CreateConVar("ray_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by ray. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another ray comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, rays do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_ray_PANIC_COUNT rays are this close, or an equivalent score.
local MUSIC_ray_PANIC_COUNT = 8 -- That's a lot of ray.

local MUSIC_ray_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_ray_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("ray") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("rayRayMusicUpdate")

		if (RayMusic ~= nil) then
			RayMusic:Stop()
		end
	...
	if (RayMusic == nil) then
		if (IsValid(LocalPlayer())) then
			RayMusic = CreateSound(LocalPlayer(), "ray/panic.mp3")
			RayMusic:Stop()
		else
	...
	if (ray_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		RayMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "ray") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_ray_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear ray taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(ray_music_volume:GetFloat(), 0, 1))

	RayMusic:Play()
	RayMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	RayMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("rayRayMusicUpdate")) then
		timer.Create("rayRayMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local rayMaterial = Material("ray/ray.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(rayMaterial)
...
hook.Add("OnEntityCreated", "rayInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "ray") then return end

	local rayEntTable = scripted_ents.GetStored("ray")

	table.Merge(ent, rayEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("ray_removed", rayDeregister)
end)

hook.Add("NetworkEntityCreated", "rayNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "ray") then return end
...
surface.CreateFont("rayHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("rayHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("ray is studying this map.", "rayHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "rayHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "rayHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "rayHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "rayHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("ray_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "rayNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. ray doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for ray to figure this map out.
...
-- Lazy
net.Receive("ray_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! ray doesn't know this map.
Would you like ray to learn it?]],
		            "This map is currently not ray-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! ray doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to ray.]],
		            "This map is currently not ray-compatible!",
		            "Ok", nagAgain,
