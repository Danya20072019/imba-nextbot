...
local dtwo_acquire_distance = CreateConVar("dtwo_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
...
function ENT:AttemptJumpAtTarget()
	...
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "dtwo/jump.mp3" or "dtwo/jump.mp3"), 350, 100)
	end
end
