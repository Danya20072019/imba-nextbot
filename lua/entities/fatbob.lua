...
util.PrecacheSound("fatbob/fart.mp3")
util.PrecacheSound("fatbob/panic.mp3")
util.PrecacheSound("fatbob/kill.mp3")
...
local fatbob_spawn_protect = CreateConVar("fatbob_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, fatbob will not target players or hide within 200 units of a spawn point.")

local fatbob_attack_distance = CreateConVar("fatbob_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of fatbob's attack.")

local fatbob_attack_interval = CreateConVar("fatbob_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between fatbob's attacks.")

local fatbob_attack_force = CreateConVar("fatbob_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of fatbob's attack. Higher values throw things farther.")

local fatbob_smash_props = CreateConVar("fatbob_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, fatbob will punch through any props placed in their way.")

local fatbob_hiding_scan_interval = CreateConVar("fatbob_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "fatbob will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant fatbobs are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local fatbob_hiding_repath_interval = CreateConVar("fatbob_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to fatbob's hiding spot will be redetermined every X seconds.")

local fatbob_chase_repath_interval = CreateConVar("fatbob_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of fatbob's target will be redetermined every X seconds.")

local fatbob_expensive_scan_interval = CreateConVar("fatbob_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local fatbob_force_download = CreateConVar("fatbob_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download fatbob resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear fatbob!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (fatbob_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("fatbob_nag")
util.AddNetworkString("fatbob_nagresponse")
util.AddNetworkString("fatbob_navgen")
...
local VECTOR_fatbob_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_fatbob_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("fatbob: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other fatbobs, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "fatbobMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "fatbob") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.fatbob_HasBeenNagged) then return end
	ply.fatbob_HasBeenNagged = true

	-- Try to explain why fatbob isn't working.
	net.Start("fatbob_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("fatbob: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("fatbob: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("fatbob_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.fatbob_HasBeenNagged = false
		return
	end
	...
	--for _, fatbob in pairs(ents.FindByClass("fatbob")) do
	--	fatbob:Remove()
	--end

	print("fatbob: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "fatbobNavGen", navEndGenerate)
	else
		print("fatbob: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = fatbob_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `fatbob_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (fatbob_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (fatbob_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. fatbob means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * fatbob_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * fatbob_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not fatbob_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (fatbob_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = fatbob_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "fatbob/fart.mp3" or "fatbob/fart.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > fatbob_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > fatbob_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(fatbob_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > fatbob_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= fatbob_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= fatbob_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= fatbob_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= fatbob_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local fatbobsLastFrame = 0
hook.Add("Tick", "fatbobBenchmark", function()
	local now = SysTime()
	local numfatbobs = #ents.FindByClass("fatbob")

	if (timeAll ~= 0) then

		local fatbobTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    fatbobTime / fatbobsLastFrame, fatbobTime, frameTime, (fatbobTime / frameTime) * 100, fatbobsLastFrame, numThink, (numThink / fatbobsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	fatbobsLastFrame = numfatbobs
end)]]
...
killicon.Add("fatbob", "fatbob/fatbob_killicon", color_white)
language.Add("fatbob", "fatbob")
...
local fatbobMusic = nil
local lastPanic = 0 -- The last time we were in music range of a fatbob.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local fatbob_music_volume = CreateConVar("fatbob_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by fatbob. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another fatbob comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, fatbobs do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_fatbob_PANIC_COUNT fatbobs are this close, or an equivalent score.
local MUSIC_fatbob_PANIC_COUNT = 8 -- That's a lot of fatbob.

local MUSIC_fatbob_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_fatbob_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("fatbob") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("fatbobfatbobMusicUpdate")

		if (fatbobMusic ~= nil) then
			fatbobMusic:Stop()
		end
	...
	if (fatbobMusic == nil) then
		if (IsValid(LocalPlayer())) then
			fatbobMusic = CreateSound(LocalPlayer(), "fatbob/panic.mp3")
			fatbobMusic:Stop()
		else
	...
	if (fatbob_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		fatbobMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "fatbob") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_fatbob_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear fatbob taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(fatbob_music_volume:GetFloat(), 0, 1))

	fatbobMusic:Play()
	fatbobMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	fatbobMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("fatbobfatbobMusicUpdate")) then
		timer.Create("fatbobfatbobMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local fatbobMaterial = Material("fatbob/fatbob.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(fatbobMaterial)
...
hook.Add("OnEntityCreated", "fatbobInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "fatbob") then return end

	local fatbobEntTable = scripted_ents.GetStored("fatbob")

	table.Merge(ent, fatbobEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("fatbob_removed", fatbobDeregister)
end)

hook.Add("NetworkEntityCreated", "fatbobNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "fatbob") then return end
...
surface.CreateFont("fatbobHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("fatbobHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("fatbob is studying this map.", "fatbobHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "fatbobHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "fatbobHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "fatbobHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "fatbobHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("fatbob_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "fatbobNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. fatbob doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for fatbob to figure this map out.
...
-- Lazy
net.Receive("fatbob_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! fatbob doesn't know this map.
Would you like fatbob to learn it?]],
		            "This map is currently not fatbob-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! fatbob doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to fatbob.]],
		            "This map is currently not fatbob-compatible!",
		            "Ok", nagAgain,
