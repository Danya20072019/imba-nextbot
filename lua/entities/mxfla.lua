...
util.PrecacheSound("maxsnd/jump.mp3")
...
local mxfla_spawn_protect = CreateConVar("mxfla_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, maxfla will not target players or hide within 200 units of a spawn point.")

local mxfla_attack_distance = CreateConVar("mxfla_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of maxfla's attack.")

local mxfla_attack_interval = CreateConVar("mxfla_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between maxfla's attacks.")

local mxfla_attack_force = CreateConVar("mxfla_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of maxfla's attack. Higher values throw things farther.")

local mxfla_smash_props = CreateConVar("mxfla_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, maxfla will punch through any props placed in their way.")

local mxfla_hiding_scan_interval = CreateConVar("mxfla_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "maxfla will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant maxflas are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local mxfla_hiding_repath_interval = CreateConVar("mxfla_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to maxfla's hiding spot will be redetermined every X seconds.")

local mxfla_chase_repath_interval = CreateConVar("mxfla_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of maxfla's target will be redetermined every X seconds.")

local mxfla_expensive_scan_interval = CreateConVar("mxfla_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local mxfla_force_download = CreateConVar("mxfla_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download maxfla resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear maxfla!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (mxfla_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("maxfla_nag")
util.AddNetworkString("maxfla_nagresponse")
util.AddNetworkString("maxfla_navgen")
...
local VECTOR_maxfla_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_maxfla_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("mxfla: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other maxflas, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "maxflaMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "mxfla") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.maxfla_HasBeenNagged) then return end
	ply.maxfla_HasBeenNagged = true

	-- Try to explain why maxfla isn't working.
	net.Start("maxfla_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("mxfla: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("mxfla: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("maxfla_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.maxfla_HasBeenNagged = false
		return
	end
	...
	--for _, maxfla in pairs(ents.FindByClass("mxfla")) do
	--	maxfla:Remove()
	--end

	print("mxfla: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "maxflaNavGen", navEndGenerate)
	else
		print("mxfla: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = mxfla_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `mxfla_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (mxfla_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (mxfla_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. maxfla means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * mxfla_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * mxfla_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not mxfla_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (mxfla_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = mxfla_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "maxsnd/jump.mp3" or "maxsnd/jump.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > mxfla_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > mxfla_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(mxfla_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > mxfla_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= mxfla_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= mxfla_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= mxfla_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= mxfla_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local maxflasLastFrame = 0
hook.Add("Tick", "maxflaBenchmark", function()
	local now = SysTime()
	local nummaxflas = #ents.FindByClass("mxfla")

	if (timeAll ~= 0) then

		local maxflaTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    maxflaTime / maxflasLastFrame, maxflaTime, frameTime, (maxflaTime / frameTime) * 100, maxflasLastFrame, numThink, (numThink / maxflasLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	maxflasLastFrame = nummaxflas
end)]]
...
killicon.Add("mxfla", "maxfla/npc_maxfla_killicon", color_white)
language.Add("mxfla", "MAX FLA")
...
local MaxPanic = nil
local lastPanic = 0 -- The last time we were in music range of a maxfla.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local mxfla_music_volume = CreateConVar("mxfla_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by maxfla. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another maxfla comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, maxflas do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_maxfla_PANIC_COUNT maxflas are this close, or an equivalent score.
local MUSIC_maxfla_PANIC_COUNT = 8 -- That's a lot of maxfla.

local MUSIC_maxfla_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_maxfla_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("mxfla") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("maxflaMaxPanicUpdate")

		if (MaxPanic ~= nil) then
			MaxPanic:Stop()
		end
	...
	if (MaxPanic == nil) then
		if (IsValid(LocalPlayer())) then
			MaxPanic = CreateSound(LocalPlayer(), "maxsnd/cover.mp3")
			MaxPanic:Stop()
		else
	...
	if (mxfla_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		MaxPanic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "mxfla") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_maxfla_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear maxfla taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(mxfla_music_volume:GetFloat(), 0, 1))

	MaxPanic:Play()
	MaxPanic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	MaxPanic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("maxflaMaxPanicUpdate")) then
		timer.Create("maxflaMaxPanicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local maxflaMaterial = Material("maxfla/mxfla.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(maxflaMaterial)
...
hook.Add("OnEntityCreated", "maxflaInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "mxfla") then return end

	local maxflaEntTable = scripted_ents.GetStored("mxfla")

	table.Merge(ent, maxflaEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("maxfla_removed", maxflaDeregister)
end)

hook.Add("NetworkEntityCreated", "maxflaNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "mxfla") then return end
...
surface.CreateFont("maxflaHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("maxflaHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("maxfla is studying this map.", "maxflaHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "maxflaHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "maxflaHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "maxflaHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "maxflaHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("maxfla_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "maxflaNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. maxfla doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for maxfla to figure this map out.
...
-- Lazy
net.Receive("maxfla_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! maxfla doesn't know this map.
Would you like maxfla to learn it?]],
		            "This map is currently not maxfla-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! maxfla doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to maxfla.]],
		            "This map is currently not maxfla-compatible!",
		            "Ok", nagAgain,
