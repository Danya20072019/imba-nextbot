...
util.PrecacheSound("endr/endr.mp3")
util.PrecacheSound("endr/idinahui.mp3")
util.PrecacheSound("endr/idinahui2.mp3")
util.PrecacheSound("endr/kill1.mp3")
util.PrecacheSound("endr/kill2.mp3")
...
local endr_spawn_protect = CreateConVar("endr_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, endr will not target players or hide within 200 units of a spawn point.")

local endr_attack_distance = CreateConVar("endr_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of endr's attack.")

local endr_attack_interval = CreateConVar("endr_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between endr's attacks.")

local endr_attack_force = CreateConVar("endr_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of endr's attack. Higher values throw things farther.")

local endr_smash_props = CreateConVar("endr_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, endr will punch through any props placed in their way.")

local endr_hiding_scan_interval = CreateConVar("endr_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "endr will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant endrs are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local endr_hiding_repath_interval = CreateConVar("endr_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to endr's hiding spot will be redetermined every X seconds.")

local endr_chase_repath_interval = CreateConVar("endr_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of endr's target will be redetermined every X seconds.")

local endr_expensive_scan_interval = CreateConVar("endr_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local endr_force_download = CreateConVar("endr_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download endr resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear endr!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (endr_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("endr_nag")
util.AddNetworkString("endr_nagresponse")
util.AddNetworkString("endr_navgen")
...
local VECTOR_endr_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_endr_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("endr: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other endrs, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "endrMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "endr") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.endr_HasBeenNagged) then return end
	ply.endr_HasBeenNagged = true

	-- Try to explain why endr isn't working.
	net.Start("endr_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("endr: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("endr: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("endr_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.endr_HasBeenNagged = false
		return
	end
	...
	--for _, endr in pairs(ents.FindByClass("endr")) do
	--	endr:Remove()
	--end

	print("endr: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "endrNavGen", navEndGenerate)
	else
		print("endr: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = endr_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `endr_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (endr_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (endr_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. endr means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * endr_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * endr_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not endr_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (endr_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = endr_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "endr/idinahui.mp3" or "endr/idinahui2.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > endr_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > endr_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(endr_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > endr_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= endr_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= endr_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= endr_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= endr_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local endrsLastFrame = 0
hook.Add("Tick", "endrBenchmark", function()
	local now = SysTime()
	local numendrs = #ents.FindByClass("endr")

	if (timeAll ~= 0) then

		local endrTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    endrTime / endrsLastFrame, endrTime, frameTime, (endrTime / frameTime) * 100, endrsLastFrame, numThink, (numThink / endrsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	endrsLastFrame = numendrs
end)]]
...
killicon.Add("endr", "endr/endr_killicon", color_white)
language.Add("endr", "Endr Port")
...
local endrMusic = nil
local lastPanic = 0 -- The last time we were in music range of a endr.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local endr_music_volume = CreateConVar("endr_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by endr. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another endr comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, endrs do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_endr_PANIC_COUNT endrs are this close, or an equivalent score.
local MUSIC_endr_PANIC_COUNT = 8 -- That's a lot of endr.

local MUSIC_endr_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_endr_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("endr") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("endrendrMusicUpdate")

		if (endrMusic ~= nil) then
			endrMusic:Stop()
		end
	...
	if (endrMusic == nil) then
		if (IsValid(LocalPlayer())) then
			endrMusic = CreateSound(LocalPlayer(), "endr/endr.mp3")
			endrMusic:Stop()
		else
	...
	if (endr_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		endrMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "endr") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_endr_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear endr taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(endr_music_volume:GetFloat(), 0, 1))

	endrMusic:Play()
	endrMusic:ChangePitch(math.Clamp(game.GetTimeScale() * 100, 50, 255), 0) -- Just for kicks.
	endrMusic:ChangeVolume(musicVolume, 0)
end

local function startTimer()
	if (not timer.Exists("endrendrMusicUpdate")) then
		timer.Create("endrendrMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local endrMaterial = Material("endr/endr.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(endrMaterial)
...
hook.Add("OnEntityCreated", "endrInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "endr") then return end

	local endrEntTable = scripted_ents.GetStored("endr")

	table.Merge(ent, endrEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("endr_removed", endrDeregister)
end)

hook.Add("NetworkEntityCreated", "endrNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "endr") then return end
...
surface.CreateFont("endrHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("endrHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("endr is studying this map.", "endrHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "endrHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "endrHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "endrHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "endrHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("endr_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "endrNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. endr doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for endr to figure this map out.
...
-- Lazy
net.Receive("endr_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! endr doesn't know this map.
Would you like endr to learn it?]],
		            "This map is currently not endr-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! endr doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to endr.]],
		            "This map is currently not endr-compatible!",
		            "Ok", nagAgain,
